...
// 1. FORCE NEXT.JS TO BYPASS ALL STATIC CACHING
export const dynamic = 'force-dynamic';

// 2. THE LIVE DATA FETCH FUNCTION
async function getBlogPosts() {
  ...
  // This third argument tells Vercel never to store/cache this data request
  return await client.fetch(query, {}, { cache: 'no-store' });
}

export default async function TaskmePreneurHome() {
  // Fetch real articles from your Sanity database
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans selection:bg-cyan-500 selection:text-slate-900 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-indigo-950/40 via-slate-950 to-slate-950">
      ...
      <main className="max-w-5xl mx-auto px-6 pb-24">
        ...
        {posts.length === 0 ? (
          <div className="text-center py-12 text-slate-500 font-mono text-sm border border-dashed border-slate-800 rounded-xl">
            No telemetry modules detected. Publish an article in Sanity Studio to populate grid.
          </div>
        ) : (
              ...
              >
                ...
                <div className="p-6 flex flex-col flex-1 relative z-10">
                  <div className="flex items-center gap-3 text-xs font-mono text-slate-500 mb-3">
                    ...
                    <span>•</span>
                    <span className="text-cyan-400/80">{post.readTime || '3 min read'}</span>
                  </div>
      ...
      <footer className="border-t border-slate-900/60 py-12 bg-slate-950/80 backdrop-blur">
        <div className="max-w-5xl mx-auto px-6 flex flex-col sm:flex-row items-center justify-between gap-6 text-xs font-mono text-slate-600">
          <p>© 2026 Taskme.preneur. Automated via Next.js & Sanity.</p>
